...
(() => {
  ...
  async function api(action='snapshot',body) {
    const res=await fetch(`/api/inventory?chickenUnit=pollos&action=${action}`,{credentials:'same-origin',method:body?'POST':'GET',headers:body?{'Content-Type':'application/json'}:{},body:body?JSON.stringify(body):undefined});
  ...
  async function load(){
    ...
    if(next){const destination=new URL(next,window.location.origin);if(destination.origin===window.location.origin&&['/dashboard.html','/preguntale.html','/supervision.html','/captura.html','/rastro.html','/proteinas.html','/proteinas-movil.html'].includes(destination.pathname)){window.location.replace(destination.href);return;}}
  ...
  function production(){
    ...
    $('#view').innerHTML=panel(location==='cedis'?'Preparación en CEDIS':'Cocción en Sucursal',can(action)?`<form id="production-form">${selectField('recipe','Proceso',options)}${quantityField('qty',0.001)}<div class="inv-note" id="production-preview"></div>${noteField(false,'Indicación de Nancy / referencia de tanda')}${submit('Registrar transformación')}</form>`:'Tu cuenta no puede registrar esta operación.');
    const form=$('#production-form');if(form){const preview=()=>{const r=form.elements.recipe.value;const q=Number(form.elements.qty.value)||0;$('#production-preview').textContent=r==='freir'?`${q} pollos marinados → ${q} pollos CRUJI cocinados`:`${q} pollos utilizados → ${q} pollos preparados`;};form.addEventListener('input',preview);preview();}
  ...
  function sales(){
    $('#view').innerHTML=panel('Registrar ventas o cortesías',can('sale')?`<form id="sale-form">${selectField('item','Producto',itemOptions(i=>i.kind==='finished'))}${quantityField('qty',0.001)}<p>Pollo: captura pollos completos o decimales. Medio = 0.5; cuarto = 0.25.</p>${selectField('kind','Tipo',[['sale','Venta'],['courtesy','Cortesía']])}<div id="sale-preview"></div>${noteField(false,'Nota / motivo de cortesía')}${submit('Registrar salida por venta')}</form>`:'Tu cuenta no puede registrar ventas.');
    const form=$('#sale-form');if(form){const preview=()=>{const id=form.elements.namedItem('item').value,product=item(id);form.elements.qty.step=product.step/1000;form.elements.qty.min=product.step/1000;$('#sale-preview').textContent=`Disponible: ${initialized('sucursal',id)?fmt(bal('sucursal',id)):'Sin saldo inicial'} ${product.unit}. Salida: ${Number(form.elements.qty.value)||0} ${product.unit}.`;};form.addEventListener('input',preview);preview();}
    wireForm('sale-form',v=>commit('sale',{presentation:'article',item:v.item,qty:Number(v.qty),kind:v.kind,note:v.note}));
  }
  function counts(){

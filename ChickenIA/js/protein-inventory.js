...
(()=>{
 ...
 const inventory=document.body.dataset.proteinInventory||'sucursal',mobile=inventory==='movil',destination=mobile?'Chicanito Móvil':'Sucursal';
 const page=mobile?'/proteinas-movil.html':'/proteinas.html';
 ...
 $('#date').max=today;
 const actions={initial:'Primera captura: lo que hay en CEDIS',send:`SALIDA A ${destination.toUpperCase()} — se resta de CEDIS`,entry:'Llegó pollo del proveedor — se suma',marinate:'Ya marinamos estos pollos — siguen en CEDIS',receive:`${destination}: confirmar lo que llegó`,count:'Contar los pollos — solo comparar',waste:'Pollo perdido o dañado — se resta',rectify:'RECTIFICAR INVENTARIO — corregir lo que hay'};
 const guidance={initial:'Escribe lo que está físicamente en CEDIS al comenzar. Separa los pollos sin marinar de los marinados. No cuentes un pollo dos veces ni incluyas los que ya salieron.',entry:`Escribe solo los pollos nuevos que trajo el proveedor a CEDIS. No escribas aquí los que salen a ${destination}.`,marinate:'Escribe cuántos pollos acabas de marinar. Pasan de sin marinar a Pollos Marinados. Todavía están en CEDIS: el total no aumenta.',send:`Escribe cuántos pollos marinados salen de CEDIS hacia ${destination}. Se RESTAN de CEDIS. No escribas cuántos quedan. Si aún aparecen sin marinar, registra primero que ya se marinaron.`,receive:`Estás confirmando en ${destination} cuántos pollos llegaron de esa entrega. Ya se descontaron al salir de CEDIS; no se vuelven a restar.`,count:'Escribe lo que contaste en CEDIS. Esto solo compara y muestra diferencias. Para corregir lo guardado, usa RECTIFICAR INVENTARIO.',waste:'Escribe cuántos pollos se perdieron o dañaron y explica qué pasó. Se restan de CEDIS.',rectify:`Cuenta los pollos que realmente quedan en CEDIS y escribe las cantidades correctas. Reemplazarán las cantidades actuales. No incluyas los que ya salieron. Explica el error. Si solo falta registrar una salida, usa el campo Lo que sale ahora a ${destination} en la ficha superior.`};
 ...
 async function api(body,day){const r=await fetch('/api/protein-inventory?inventory='+inventory+'&date='+encodeURIComponent(day),body?{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)}:{cache:'no-store'});if(r.status===401){location.assign('/inventario.html?next='+encodeURIComponent(page+'?date='+day));throw Error('Inicia sesión para continuar.');}const data=await r.json();if(!r.ok)throw Error(data.error);if(data.inventory!==inventory)throw Error('El inventario recibido no corresponde a este apartado. Recarga.');return data;}
 function form(){
  if(!snapshot)return;
 ...
 function fields(){
  ...
  const labels={send:`¿Cuántos pollos salen a ${destination}?`,entry:'¿Cuántos pollos nuevos trajo el proveedor?',marinate:'¿Cuántos pollos acabas de marinar?',receive:`¿Cuántos pollos llegaron a ${destination}?`,waste:'¿Cuántos pollos se perdieron o dañaron?'};
 ...
 function preview(){
  ...
  $('#preview').textContent=action==='send'?`En CEDIS hay ${view.fmt(r.total)} − salen ${view.fmt(n)} = quedan ${view.fmt(round(raw+mar))} pollos. De los ${view.fmt(r.marinated.current)} marinados quedarán ${view.fmt(round(mar))}.`:
   action==='receive'?`Se guardará lo recibido en ${destination}, tu nombre y la hora.`:`Quedarán en CEDIS: ${view.fmt(round(raw))} sin marinar + ${view.fmt(round(mar))} marinados = ${view.fmt(round(raw+mar))} pollos.`;

...
const qty=z.number().min(0).max(1000000).multipleOf(0.001);
...
async function read(query,day){
  ...
  const lines=items.map(item=>{
    ...
    const final=known?(amount('initial')+amount('entry')-amount('exit'))/1000:null;
    return {...item,initialized:!!initial,initialDate:initial?String(initial.movement_date).slice(0,10):null,previous:final===null?null:Math.round((final-onDay('entry')+onDay('exit'))*1000)/1000,entry:onDay('entry'),exit:onDay('exit'),final};
  });
  return {date:day,locationId:location.id,revision,lines,history:movements.filter(m=>String(m.movement_date).slice(0,10)<=day).slice(-200).reverse()};
}
async function save(query,body,user){
  ...
  for(const l of d.lines){
    ...
    let balance=0;
    for(const row of rows){if(!['initial','entry','exit'].includes(row.movement_type))fail('Hay movimientos de otro formato; requiere revisión.',409);balance+=Math.round(Number(row.quantity)*1000)*(row.movement_type==='exit'?-1:1);if(balance<0)fail('La salida supera la existencia de '+item.name+'.');}
  }

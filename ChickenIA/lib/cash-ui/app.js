...
const steps=()=>mode==='close'?['Efectivo','Conteo','Entrega','Ventas','Revisión']:['Cocina','Freidoras','Rosticero',state?.data?.photo?'Mermas y recepción':'Compras y comida'];
...
async function load(){lock(true);$('#error').textContent='';try{state=await api(`/api/${mode}?date=${encodeURIComponent($('#date').value)}`);dirty=false;initPhoto();render();}catch(e){state=null;$('#fields').innerHTML='';failure(e);}finally{lock(false);}}
function initPhoto(){
...
function cashForm(){
  if(state.data.photo&&step===0)return `<legend>Dinero del cierre fotografiado</legend><div class="fields">${input('cashier','Prepara el cierre')}${input('cut','Referencia del corte')}${['expenses','firstTurn','retained','delivered'].map(k=>input('cash.'+k,cashLabels[k]+' ($)','money')).join('')}</div>`;
  if(state.data.photo&&step===1)return `<legend>Billetes y total de monedas</legend><div class="fields">${Object.entries(denominations).filter(([k])=>k.startsWith('b')).map(([k,v])=>input('counts.'+k,money(v),'count')).join('')}${input('photo.coinsTotal','Monedas: total ($)','money')}</div>`;
  if(state.data.photo&&step===3)return `<legend>Otros pagos</legend><div class="fields">${input('photo.cardTotal','Pago con tarjeta: total ($)','money')}${input('payments.transfer.confirmed','Pago por transferencia: total ($)','money')}</div>`;
...
function productionForm(){
  if(state.data.photo&&step===0)return '<legend>Cocina</legend><p>Esta hoja corresponde a proteínas y caja. La producción de cocina se captura en su formato propio.</p>';
  if(state.data.photo&&step===3)return `<legend>Recepción, mermas y aclaraciones</legend><div class="fields">${input('photo.deliveredBy','Entregó la proteína')}${input('photo.receivedBy','Recibió la proteína')}${[0,1,2].map(i=>input('photo.batchTimes.'+i,'Hora de producción '+(i+1))).join('')}${area('notes','Aclaraciones')}</div>${details('losses','Mermas y consumo')}`;
  ...
  const currentArea=steps()[step],stageFields=state.data.photo?(productionStage==='1'?['previousRaw','previousCooked','receivedRaw','done1']:productionStage==='2'?['done2']:productionStage==='third'?['done3']:['closingCooked','closingRaw']):productionStage==='1'?['previousRaw','previousCooked','plan1','done1']:productionStage==='2'?['plan2','done2']:['closingCooked','closingRaw'];

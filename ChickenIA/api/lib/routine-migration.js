...
module.exports = async function migrateRoutines(sql, weights) {
  ...
  const records = routines.map(([name, criticality, requires_quantity, unit, indicator_type, target, routine_block], i) => ({
    name, criticality, weight: weights[criticality], requires_quantity, unit,
    indicator_type, target, routine_block, order_index: i + 1,
  }));
  ...
  await sql`
    WITH version AS (
      ...
      SELECT 'rosticero-routines-2026-09-11', (now() AT TIME ZONE 'America/Mexico_City')::date
        + CASE WHEN EXISTS (
          SELECT 1 FROM activity_checks c JOIN activities a ON a.id=c.activity_id
          JOIN areas ar ON ar.id=a.area_id WHERE ar.code='rosticero'
            AND c.check_date=(now() AT TIME ZONE 'America/Mexico_City')::date
        ) THEN 1 ELSE 0 END
      WHERE EXISTS (SELECT 1 FROM areas WHERE code = 'rosticero')
      ON CONFLICT DO NOTHING RETURNING effective_date
    ), retired AS (
      UPDATE activities SET valid_until = version.effective_date
      FROM areas, version
      WHERE activities.area_id = areas.id AND areas.code = 'rosticero'
    ...
    INSERT INTO activities (area_id, name, criticality, weight, requires_quantity,
      unit, indicator_type, target, routine_block, order_index, valid_from)
    SELECT areas.id, r.name, r.criticality, r.weight, r.requires_quantity,
      r.unit, r.indicator_type, r.target, r.routine_block, r.order_index, version.effective_date
    FROM areas CROSS JOIN version CROSS JOIN jsonb_to_recordset(${JSON.stringify(records)}::jsonb)
      AS r(name TEXT, criticality TEXT, weight INT, requires_quantity BOOLEAN,
        unit TEXT, indicator_type TEXT, target TEXT, routine_block TEXT, order_index INT)
    WHERE areas.code = 'rosticero'
  `;
};

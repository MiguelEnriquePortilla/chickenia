...
module.exports = async (req, res) => {
  try {
    ...
    if (req.method === 'POST') {
      ...
      const proteinItems=await sql`SELECT sku FROM inventory_items WHERE id=${item_id}`;
      if(proteinItems[0]?.sku?.startsWith('PRO-'))return res.status(409).json({error:'Registra proteínas desde Inventario de Proteínas.'});
